...
COMMENT ON TABLE public.push_tokens IS 'Device push notification tokens for users';
...
COMMENT ON FUNCTION get_active_push_tokens(UUID) IS 'Get active push tokens for a user (max 5, recent only)';
COMMENT ON FUNCTION log_notification(UUID, TEXT, TEXT, TEXT, JSONB, UUID, TEXT) IS 'Log a notification that was sent';
COMMENT ON FUNCTION get_user_notification_settings(UUID) IS 'Get notification settings for a user';
COMMENT ON FUNCTION cleanup_old_push_tokens() IS 'Remove inactive push tokens older than 90 days';
