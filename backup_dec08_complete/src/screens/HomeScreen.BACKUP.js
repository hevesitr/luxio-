...
const HomeScreen = ({ onMatch, navigation, matches = [], route }) => {
  ...
  const handleSwipeRight = useCallback(async (profile) => {
    ...
    try {
      ...
      if (result.isMatch) {
        ...
        // ✅ Add match to app state via onMatch callback
        if (onMatch) {
          onMatch(profile);
        }
        ...
        setTimeout(() => {
          console.log('HomeScreen: Hiding match animation and clearing matchedProfile');
      ...
      console.error('HomeScreen swipe error:', error);
    }
  }, [user?.id]);
  ...
  const handleSuperLike = useCallback(async (profile) => {
    ...
    try {
      ...
      // ✅ Check if swipe was successful
      if (!result || !result.success) {
        Logger.error('HomeScreen: Super like failed', { error: result?.error, profile: profile?.name });
        console.error('Super like error:', result?.error);
        return;
      }
      ...
      console.error('HomeScreen superlike error:', error);
    }
  }, [user?.id]);
  ...
  // ✅ FIX: Auto-load more profiles when running low
  useEffect(() => {
    const loadMoreProfilesIfNeeded = async () => {
      const remainingProfiles = dataState.profiles.length - dataState.currentIndex;
      if (remainingProfiles <= 3 && !uiState.loading) { // Load when 3 or fewer profiles remain
        console.log('HomeScreen: Running low on profiles, loading more...', { remaining: remainingProfiles });

        try {
          const excludeIds = history.map(h => h.id);
          const newProfiles = await DiscoveryService.getDiscoveryProfiles(filterState.searchFilters, excludeIds).catch(() => []);

          if (newProfiles.length > 0) {
            updateDataState(prev => ({
              profiles: [...prev.profiles, ...newProfiles]
            }));
            console.log('HomeScreen: Loaded', newProfiles.length, 'additional profiles');
          } else {
            console.log('HomeScreen: No more profiles available with current filters');
          }
        } catch (error) {
          console.error('HomeScreen: Error loading more profiles:', error);
        }
      }
    };

    loadMoreProfilesIfNeeded();
  }, [dataState.currentIndex, dataState.profiles.length, history, filterState.searchFilters, uiState.loading]);
  ...
  useEffect(() => {
    if (__DEV__) {
      ...
      console.log('HomeScreen: First 5 profile IDs:', dataState.profiles.slice(0, 5).map(p => `${p.id}:${p.name}`).join(', '));
    }
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]} edges={['top', 'left', 'right']}>
      ...
      {/* Match Animation */}
      <MatchAnimation
        key={dataState.matchedProfile?.id || 'no-match'} // Force re-render for each new match
        visible={uiState.matchAnimVisible}
        profile={dataState.matchedProfile}
        onClose={() => {
          console.log('MatchAnimation closed, clearing matchedProfile');
          updateUiState({ matchAnimVisible: false });
          updateDataState({ matchedProfile: null });
        }}
      />
