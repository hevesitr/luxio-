...
class AccountService {
  ...
  async updateCookieSettings(settings) {
    try {
      ...
      if (error) {
        // Handle table not found error (Phase 1 not deployed yet)
        if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
          Logger.warn('Cookie settings table not found - Phase 1 deployment needed, storing locally only');
        } else {
          throw error;
        }
      }
  ...
  async getCookieSettings(userId) {
    try {
      ...
      if (error && error.code !== 'PGRST116') {
        // Handle table not found error (Phase 1 not deployed yet)
        if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
          Logger.warn('Cookie settings table not found - Phase 1 deployment needed, using defaults');
        } else {
          throw error;
        }
      }
