...
// Phase 1: Offline Queue for network resilience
...
class MatchService extends BaseService {
  constructor() {
    ...
    // Phase 1: Setup network reconnection listener
    this.setupNetworkListener();
  }
  
  /**
   * Phase 1: Setup network reconnection listener for offline queue sync
   */
  setupNetworkListener() {
    NetInfo.addEventListener(state => {
      if (state.isConnected && state.isInternetReachable) {
        Logger.info('[MatchService] Network reconnected - triggering queue sync');
        // Trigger sync for all users (in real app, would be current user only)
  ...
  /**
   * Phase 1: Sync offline queue when network is restored
   */
  async syncOfflineQueue() {
    try {
      // In a real app, we'd get the current user ID
      // For now, we'll just log that sync would happen
      Logger.info('[MatchService] Offline queue sync triggered');
      // offlineQueueService.syncQueue(currentUserId) would be called here
    } catch (error) {
      Logger.error('[MatchService] Error syncing offline queue', error);
    }
  }
    ...
    try {
      ...
      if (jsonValue != null) {
        ...
        if (Array.isArray(matches)) {
          ...
          console.warn('MatchService: Invalid matches data, returning empty array');
          return [];
        }
      }
      return [];
   ...
   * Match hozzáadása
   */
  async addMatch(profile, userId) {
    try {
      const matches = await this.loadMatches(userId);
      ...
      await this.saveMatches(matches, userId);
      return matches;
    } catch (error) {
   ...
   * Match törlése
   */
  async removeMatch(profileId, userId) {
    try {
      const matches = await this.loadMatches(userId);
      const filtered = matches.filter(m => m.id !== profileId);
      await this.saveMatches(filtered, userId);
    ...
    try {
      ...
      if (jsonValue != null) {
        ...
        if (Array.isArray(history)) {
          ...
          console.warn('MatchService: Invalid history data, returning empty array');
          return [];
        }
      }
      return [];
  ...
  async processSwipe(userId, targetUserId, action, userPreferences = {}) {
    try {
      ...
      if (!isOnline) {
        // Phase 1: Queue action for later sync
        Logger.info('[MatchService] Offline - queueing swipe action', { userId, targetUserId, action });
        await offlineQueueService.enqueue('swipe', {
          userId,
          targetUserId,
          action,
          userPreferences,
        ...
        return {
          success: true,
          queued: true,
          message: 'Művelet sorba állítva. Szinkronizálás online állapotban.'
        };
      }
