...
class RateLimitService {
  ...
  static async checkMessageSend(userId) {
    ...
    const attempts = await this.getAttempts(key, RateLimitService.LIMITS.MESSAGE_WINDOW_MINUTES);
    
    const allowed = attempts.length < RateLimitService.LIMITS.MESSAGE_SENDS;
    const remaining = Math.max(0, RateLimitService.LIMITS.MESSAGE_SENDS - attempts.length);

    if (allowed) {
      await this.addAttempt(key, RateLimitService.LIMITS.MESSAGE_WINDOW_MINUTES);
    }
    ...
    return {
      ...
      remaining,
      attempts: attempts.length,
      resetAt: Date.now() + (RateLimitService.LIMITS.MESSAGE_WINDOW_MINUTES * 60 * 1000),
    };
  }
